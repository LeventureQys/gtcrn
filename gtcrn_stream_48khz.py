...
class StreamGTCRN(nn.Module):
    def __init__(self):
        super().__init__()
        # For 48kHz: 1536 FFT -> 769 bins
        # ERB compression: 769 bins -> ~385 bins (keeping similar compression ratio)
        # erb_subband_1: 195 (low freq bins kept as-is)
        # erb_subband_2: 190 (ERB compressed bins)
        # Total after ERB: 195 + 190 = 385 bins
        # After 2x freq downsampling: 385/4 = 96.25 -> 97 bins
        self.erb = ERB(195, 190)
        self.sfe = SFE(3, 1)

        self.encoder = StreamEncoder()

        self.dpgrnn1 = DPGRNN(16, 97, 16)
        self.dpgrnn2 = DPGRNN(16, 97, 16)
    ...
    def forward(self, spec, conv_cache, tra_cache, inter_cache):
        """
        spec: (B, F, T, 2) = (1, 769, 1, 2)  # For 48kHz
        conv_cache: [en_cache, de_cache], (2, B, C, 8(kT-1), F) = (2, 1, 16, 16, 97)
        tra_cache: [en_cache, de_cache], (2, 3, 1, B, C) = (2, 3, 1, 1, 16)
        inter_cache: [cache1, cache2], (2, 1, BF, C) = (2, 1, 97, 16)
        """
        ...
        feat = self.erb.bm(feat)  # (B,3,T,385)
        feat = self.sfe(feat)     # (B,9,T,385)

        feat, en_outs, conv_cache[0], tra_cache[0] = self.encoder(feat, conv_cache[0], tra_cache[0])

        feat, inter_cache[0] = self.dpgrnn1(feat, inter_cache[0]) # (B,16,T,97)
        feat, inter_cache[1] = self.dpgrnn2(feat, inter_cache[1]) # (B,16,T,97)
...
if __name__ == "__main__":
    ...
    from gtcrn_48khz import GTCRN
    from modules.convert import convert_to_stream
    
    device = torch.device("cpu")

    model = GTCRN().to(device).eval()
    model.load_state_dict(torch.load('onnx_models/best_model_002.tar', map_location=device)['model'])
    stream_model = StreamGTCRN().to(device).eval()
    convert_to_stream(stream_model, model)
    
    """Streaming Conversion"""
    ### offline inference (48kHz)
    audio, sr = sf.read('test_wavs/1.wav', dtype='float32')
    if audio.ndim == 2:  # 立体声转单声道
        audio = audio.mean(axis=1)
    x = torch.from_numpy(audio)
    print(f"Audio shape: {x.shape}, Sample rate: {sr}")
    x = torch.stft(x, 1536, 768, 1536, torch.hann_window(1536).pow(0.5), return_complex=False)
    print(f"STFT output shape: {x.shape}")
    x = x.unsqueeze(0)  # 添加batch维度 (1, F, T, 2)
    with torch.no_grad():
        y = model(x)  # 输出 (B, F, T, 2)
    # 转换为复数格式用于istft
    y_complex = torch.complex(y[..., 0], y[..., 1])  # (B, F, T)
    y = torch.istft(y_complex.squeeze(0), 1536, 768, 1536, torch.hann_window(1536).pow(0.5)).detach().cpu().numpy()
    sf.write('test_wavs/enh.wav', y.squeeze(), 48000)

    ### online (streaming) inference (48kHz)
    conv_cache = torch.zeros(2, 1, 16, 16, 97).to(device)
    tra_cache = torch.zeros(2, 3, 1, 1, 16).to(device)
    inter_cache = torch.zeros(2, 1, 97, 16).to(device)
    # ys = []
    # times = []
    ...
    # ys = torch.istft(ys, 1536, 768, 1536, torch.hann_window(1536).pow(0.5)).detach().cpu().numpy()
    # sf.write('test_wavs/enh_stream.wav', ys.squeeze(), 48000)
    ...
    ## convert to onnx (48kHz)
    file = 'onnx_models/gtcrn_48k.onnx'
    if not os.path.exists(file):
        input = torch.randn(1, 769, 1, 2, device=device)
    ...
    conv_cache = np.zeros([2, 1, 16, 16, 97],  dtype="float32")
    tra_cache = np.zeros([2, 3, 1, 1, 16],  dtype="float32")
    inter_cache = np.zeros([2, 1, 97, 16],  dtype="float32")
    ...
    enhanced = istft(outputs[...,0] + 1j * outputs[...,1], n_fft=1536, hop_length=768, win_length=1536, window=np.hanning(1536)**0.5)
    sf.write('test_wavs/enh_onnx.wav', enhanced.squeeze(), 48000)
    ...
    print(">>> RTF:", 1e3*np.mean(T_list) / 16)  # RTF calculation for 48kHz (16ms per frame)
