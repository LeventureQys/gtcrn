...
CORRECTED: Frame 6 in Python = Frame 6 in C (both 0-indexed after accounting for zero padding)
"""
import os
import sys
import numpy as np
...
def main():
    print("=== Debug Decoder GTConvBlock 0 (CORRECTED Frame) ===\n")
    ...
    # CORRECTED: Frame 6 in Python (0-indexed) = Frame 6 in C (1-indexed)
    # C Frame 6 means the 6th call to process_frame, which processes:
    #   stft_window = [audio[4*256:5*256], audio[5*256:6*256]]
    # Python Frame 6 (with center=True on audio_padded) uses:
    #   audio_padded[6*256:6*256+512] = audio_padded[1536:2048]
    #   = audio[1280:1792] (since audio_padded = [zeros(256), audio])
    # Wait, that's different!

    # Actually, let me trace this more carefully:
    # Python with center=True pads with n_fft//2=256 on each side
    # So effective input = [zeros(256), audio_padded, zeros(256)]
    #                    = [zeros(256), zeros(256), audio, zeros(256)]
    #                    = [zeros(512), audio, zeros(256)]
    # Frame i uses effective_input[i*hop : i*hop + n_fft]
    # Frame 6 uses effective_input[1536:2048]
    # Since effective_input = [zeros(512), audio, zeros(256)]:
    #   indices 1536:2048 = audio[1024:1536]
    #
    # C Frame 6 (1-indexed) processes:
    #   stft_input_buffer at start of frame 6 = audio[4*256:5*256] = audio[1024:1280]
    #   current input = audio[5*256:6*256] = audio[1280:1536]
    #   stft_window = [audio[1024:1280], audio[1280:1536]] = audio[1024:1536]
    #
    # These match! Python Frame 6 = C Frame 6

    target_frame = 6  # Correct frame for comparison with C Frame 6

    # First process frames 0-5 to build up state
    with torch.no_grad():
        ...
        # Now manually trace frame 6
        frame = spec[:, :, target_frame:target_frame+1, :]
        print(f"\n=== Python Frame {target_frame} (= C Frame {target_frame}) ===")

        # Check STFT input matches
        spec_real = frame[..., 0]
        spec_imag = frame[..., 1]
        print(f"\nSTFT input check:")
        print(f"  real sum: {spec_real.sum().item():.6f}")
        print(f"  imag sum: {spec_imag.sum().item():.6f}")
        print(f"  (C Frame 6 shows: real=0.067720, imag=-0.022344)")
        ...
        print(f"\nAfter split:")
        print(f"  x1 sum: {x1.sum().item():.6f} (C shows: 99.340566)")
        print(f"  x2 sum: {x2.sum().item():.6f} (C shows: 14.804684)")

        # SFE
        x1_sfe = gt0.sfe(x1)  # (1, 24, 1, 33)
        print(f"\nAfter SFE on x1:")
        print(f"  shape: {x1_sfe.shape}")
        print(f"  sum: {x1_sfe.sum().item():.6f} (C shows: 302.950268)")

        # Point conv 1
        h1 = gt0.point_act(gt0.point_bn1(gt0.point_conv1(x1_sfe)))  # (1, 16, 1, 33)
        print(f"\nAfter point_conv1 + bn + prelu:")
        print(f"  shape: {h1.shape}")
        print(f"  sum: {h1.sum().item():.6f} (C shows: 7.744679)")
