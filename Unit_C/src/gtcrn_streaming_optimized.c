/**
 * gtcrn_streaming_optimized.c - FIXED VERSION
 *
 * Fixed issues:
 * 1. Skip connections memory management - now uses persistent buffers
 * 2. DPGRNN cache - uses GTCRNStreaming's persistent buffer instead of static
 * 3. Proper memory lifecycle management
 */
 ...
 * FIXED: Uses persistent skip buffers from GTCRNStreaming
 */
static int encoder_forward_streaming(
    const Tensor* input,        // (1, 9, 1, 385) - single frame
    Tensor* output,             // (1, 16, 1, 97)
    GTCRNStreaming* stream,     // FIXED: Pass stream to access skip_buffers
    Encoder* encoder
) {
 ...
 * FIXED: Uses persistent cache buffer from DPGRNNCache instead of static variable
 */
...
int gtcrn_streaming_process_frame_optimized(
    ...
    if (stream->model->encoder) {
        encoder_forward_streaming(&sfe_tensor, &encoder_out, stream, stream->model->encoder);
    }
    ...
    if (stream->model->dpgrnn2 && stream->dpgrnn2_cache) {
        dpgrnn_forward_streaming_wrapper(&dpgrnn1_out, &dpgrnn2_out,
    ...
    if (stream->model->decoder) {
        decoder_forward_streaming(&dpgrnn2_out, stream, &decoder_out, stream->model->decoder);
    }
...
int gtcrn_streaming_process_chunk_optimized(
    ...
    for (int i = 0; i < chunk_size; i++) {
        ...
        if (stream->stft_buffer_pos >= n_fft) {
            ...
            // Perform STFT
            stft_forward(stream->stft_input_buffer, n_fft, spec_real, spec_imag,
                        stream->stft_params);

            // Process frame through GTCRN
            gtcrn_streaming_process_frame_optimized(stream, spec_real, spec_imag,
                                                   out_real, out_imag);
