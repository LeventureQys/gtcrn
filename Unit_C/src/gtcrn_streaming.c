...
GRUCache* gru_cache_create(int hidden_size) {
    GRUCache* cache = (GRUCache*)malloc(sizeof(GRUCache));
...
TRACache* tra_cache_create(int channels) {
    TRACache* cache = (TRACache*)malloc(sizeof(TRACache));
    if (!cache) return NULL;
...
ConvCache* conv_cache_create(int channels, int cache_frames, int freq_bins) {
    ConvCache* cache = (ConvCache*)malloc(sizeof(ConvCache));
    if (!cache) return NULL;
...
DPGRNNCache* dpgrnn_cache_create(int hidden_size, int batch_size, int freq_bins) {
    ...
    cache->inter_gru_g1_cache = gru_cache_create(hidden_size / 2);
    cache->inter_gru_g2_cache = gru_cache_create(hidden_size / 2);
...
void dpgrnn_cache_free(DPGRNNCache* cache) {
    if (cache) {
        gru_cache_free(cache->inter_gru_g1_cache);
        gru_cache_free(cache->inter_gru_g2_cache);
...
void dpgrnn_cache_reset(DPGRNNCache* cache) {
    if (cache) {
        gru_cache_reset(cache->inter_gru_g1_cache);
        gru_cache_reset(cache->inter_gru_g2_cache);
...
GTCRNStreaming* gtcrn_streaming_create(
    ...
    GTCRNStreaming* stream = (GTCRNStreaming*)malloc(sizeof(GTCRNStreaming));
    ...
    stream->stft_params = stft_create(stream->n_fft, stream->hop_length, sample_rate);
    ...
    stream->encoder_conv1_cache = conv_cache_create(16, 4, 385);

    // Conv2: kernel=(1,5), dilation=(1,1) -> cache_frames = 4
    stream->encoder_conv2_cache = conv_cache_create(16, 4, 193);

    // GTConv TRA caches
    stream->encoder_gtconv1_tra_cache = tra_cache_create(8);  // 16/2 = 8
    stream->encoder_gtconv2_tra_cache = tra_cache_create(8);
    stream->encoder_gtconv3_tra_cache = tra_cache_create(8);
    ...
    stream->decoder_gtconv1_tra_cache = tra_cache_create(8);
    stream->decoder_gtconv2_tra_cache = tra_cache_create(8);
    stream->decoder_gtconv3_tra_cache = tra_cache_create(8);
    stream->decoder_conv1_cache = conv_cache_create(16, 4, 97);
    stream->decoder_conv2_cache = conv_cache_create(2, 4, 193);
    ...
    printf("GTCRN Streaming created:\n");
...
void gtcrn_streaming_free(GTCRNStreaming* stream) {
    ...
    // Free STFT
    stft_free(stream->stft_params);
    free(stream->stft_input_buffer);
    free(stream->istft_overlap_buffer);

    // Free encoder caches
    conv_cache_free(stream->encoder_conv1_cache);
    conv_cache_free(stream->encoder_conv2_cache);
    tra_cache_free(stream->encoder_gtconv1_tra_cache);
    tra_cache_free(stream->encoder_gtconv2_tra_cache);
    tra_cache_free(stream->encoder_gtconv3_tra_cache);

    // Free DPGRNN caches
    dpgrnn_cache_free(stream->dpgrnn1_cache);
    dpgrnn_cache_free(stream->dpgrnn2_cache);

    // Free decoder caches
    tra_cache_free(stream->decoder_gtconv1_tra_cache);
    tra_cache_free(stream->decoder_gtconv2_tra_cache);
    tra_cache_free(stream->decoder_gtconv3_tra_cache);
    conv_cache_free(stream->decoder_conv1_cache);
    conv_cache_free(stream->decoder_conv2_cache);
...
void gtcrn_streaming_reset(GTCRNStreaming* stream) {
    ...
    conv_cache_reset(stream->encoder_conv1_cache);
    conv_cache_reset(stream->encoder_conv2_cache);
    tra_cache_reset(stream->encoder_gtconv1_tra_cache);
    tra_cache_reset(stream->encoder_gtconv2_tra_cache);
    tra_cache_reset(stream->encoder_gtconv3_tra_cache);

    // Reset DPGRNN caches
    dpgrnn_cache_reset(stream->dpgrnn1_cache);
    dpgrnn_cache_reset(stream->dpgrnn2_cache);

    // Reset decoder caches
    tra_cache_reset(stream->decoder_gtconv1_tra_cache);
    tra_cache_reset(stream->decoder_gtconv2_tra_cache);
    tra_cache_reset(stream->decoder_gtconv3_tra_cache);
    conv_cache_reset(stream->decoder_conv1_cache);
    conv_cache_reset(stream->decoder_conv2_cache);

    // Reset statistics
    stream->frames_processed = 0;
    stream->avg_latency_ms = 0.0f;

    printf("GTCRN Streaming reset\n");
}
...
int gtcrn_streaming_process_frame(
    ...
    int freq_bins = stream->n_fft / 2 + 1;  // 769

    // Prepare input tensor (B=1, F=769, T=1, 2)
...
int gtcrn_streaming_process_chunk(
    ...
    for (int i = 0; i < chunk_size; i++) {
        ...
        if (stream->stft_buffer_pos >= n_fft) {
            ...
            stft_forward(stream->stft_input_buffer, n_fft, spec_real, spec_imag, stream->stft_params);
            ...
            gtcrn_streaming_process_frame(stream, spec_real, spec_imag, out_real, out_imag);

            // Perform iSTFT
            float* frame_audio = (float*)malloc(n_fft * sizeof(float));
            istft_forward(out_real, out_imag, 1, frame_audio, stream->stft_params);
...
int gtcrn_streaming_flush(
    ...
    if (remaining > 0) {
        ...
        // Process final frame
        gtcrn_streaming_process_chunk(stream, stream->stft_input_buffer, output);
