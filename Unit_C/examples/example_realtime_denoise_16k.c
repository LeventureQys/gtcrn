/**
 * example_realtime_denoise_16k.c - Complete example of real-time audio denoising for 16kHz
 *
 * This example demonstrates:
 * 1. Loading GTCRN model weights
 * 2. Creating streaming processor for 16kHz audio
 * 3. Processing audio file in real-time chunks
 * 4. Saving enhanced audio
 *
 * Compile:
 *   gcc -o denoise_16k example_realtime_denoise_16k.c \
 *       gtcrn_streaming_optimized_16k.c gtcrn_streaming_16k.c gtcrn_streaming_impl.c \
 *       gtcrn_model.c gtcrn_modules.c stream_conv.c stft_16k.c weight_loader.c \
 *       GRU.c conv2d.c batchnorm2d.c nn_layers.c layernorm.c -lm -O2
 *
 * Usage:
 *   ./denoise_16k input.wav output.wav weights/
 */
...
int main(int argc, char** argv) {
    if (argc < 4) {
        printf("Usage: %s <input.wav> <output.wav> <weights_dir>\n", argv[0]);
        printf("\n");
        printf("Example:\n");
        printf("  %s noisy_16k.wav clean_16k.wav weights/\n", argv[0]);
        printf("\n");
        printf("This program performs real-time audio denoising using GTCRN model at 16kHz.\n");
        return 1;
    }
    ...
    printf("\n");
    printf("=================================================================\n");
    printf("GTCRN Real-Time Audio Denoising - 16kHz Version\n");
    printf("=================================================================\n\n");
    ...
    // Verify sample rate
    if (input_audio->sample_rate != 16000) {
        fprintf(stderr, "Warning: Input sample rate is %d Hz, expected 16000 Hz\n",
                input_audio->sample_rate);
        fprintf(stderr, "         Processing may not work correctly.\n");
    }
    printf("\n");
    ...
    // Step 4: Create streaming processor for 16kHz
    printf("Step 4: Creating streaming processor for 16kHz...\n");
    int chunk_size = 256;  // 16ms @ 16kHz (was 768 for 48kHz)
    GTCRNStreaming_16k* stream = gtcrn_streaming_16k_create(model, input_audio->sample_rate, chunk_size);
    if (!stream) {
    ...
    for (int chunk = 0; chunk < total_chunks; chunk++) {
        ...
        // Process chunk
        gtcrn_streaming_16k_process_chunk_optimized(
            stream,
    ...
    float avg_latency;
    gtcrn_streaming_16k_get_stats(stream, &frames_processed, &avg_latency);
    printf("  Frames processed: %d\n", frames_processed);
    printf("  Average latency: %.2f ms\n", avg_latency);
    printf("  Total latency: %.2f ms\n", gtcrn_streaming_16k_get_latency_ms(stream));
    printf("\n");
    ...
    // Cleanup
    printf("Cleaning up...\n");
    gtcrn_streaming_16k_free(stream);
    gtcrn_free(model);
    free_audio(input_audio);
