...
if __name__ == "__main__":
    ...
    # For 48kHz: 769 frequency bins, 63 time frames
    flops, params = get_model_complexity_info(model, (769, 63, 2), as_strings=True,
                                           print_per_layer_stat=True, verbose=True)
    print(flops, params)

    """causality check"""
    # For 48kHz: 48000 samples = 1 second
    ...
    # STFT parameters for 48kHz: n_fft=1536, hop=768
    x1 = torch.stft(x1, 1536, 768, 1536, torch.hann_window(1536).pow(0.5), return_complex=False)
    x2 = torch.stft(x2, 1536, 768, 1536, torch.hann_window(1536).pow(0.5), return_complex=False)
    y1 = model(x1)[0]
    y2 = model(x2)[0]
    y1 = torch.istft(y1, 1536, 768, 1536, torch.hann_window(1536).pow(0.5), return_complex=False)
    y2 = torch.istft(y2, 1536, 768, 1536, torch.hann_window(1536).pow(0.5), return_complex=False)

    print((y1[:48000-768*2] - y2[:48000-768*2]).abs().max())
    print((y1[48000:] - y2[48000:]).abs().max())
