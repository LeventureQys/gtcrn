"""
Debug grouped convolution inputs.
Compare C and Python inputs for EnConv1 grouped conv at oc=8, of=0.
"""
import os
import sys
import numpy as np
import torch
import soundfile as sf

gtcrn_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, gtcrn_dir)

from gtcrn_stream import StreamGTCRN
from modules.convert import convert_to_stream
from gtcrn import GTCRN

def main():
    # Load model
    model_path = os.path.join(gtcrn_dir, 'checkpoints/model_trained_on_dns3.tar')
    offline_model = GTCRN().eval()
    checkpoint = torch.load(model_path, map_location='cpu', weights_only=False)
    offline_model.load_state_dict(checkpoint['model'])
    stream_model = StreamGTCRN().eval()
    convert_to_stream(stream_model, offline_model)

    # Load test audio with padding
    test_wav = os.path.join(gtcrn_dir, 'test_wavs/noisy_16k/00027_1_keyboard_loud_snr+5dB_noisy.wav')
    audio, sr = sf.read(test_wav)
    audio_padded = np.concatenate([np.zeros(256), audio])

    # STFT parameters
    n_fft = 512
    hop_length = 256
    win_length = 512
    window = torch.sqrt(torch.hann_window(win_length))

    # STFT with center=False
    audio_tensor = torch.from_numpy(audio_padded.astype(np.float32))
    spec = torch.stft(audio_tensor, n_fft=n_fft, hop_length=hop_length,
                      win_length=win_length, window=window, return_complex=False,
                      center=False)
    spec = spec.unsqueeze(0)  # (1, freq, time, 2)

    # Initialize caches
    conv_cache = torch.zeros(2, 1, 16, 16, 33)
    tra_cache = torch.zeros(2, 3, 1, 1, 16)
    inter_cache = torch.zeros(2, 1, 33, 16)

    # Process 5 frames to warm up
    with torch.no_grad():
        for i in range(5):
            frame = spec[:, :, i:i+1, :]
            out, conv_cache, tra_cache, inter_cache = stream_model(
                frame, conv_cache, tra_cache, inter_cache
            )

        # Frame 5: manual trace
        frame = spec[:, :, 5:6, :]
        x = frame  # (1, 257, 1, 2)

        # Create feature tensor (mag, real, imag)
        mag = torch.sqrt(x[..., 0]**2 + x[..., 1]**2 + 1e-12)  # (1, 257, 1)
        feat = torch.stack([mag, x[..., 0], x[..., 1]], dim=1)  # (1, 3, 257, 1)
        feat = feat.permute(0, 1, 3, 2)  # (1, 3, 1, 257)

        # ERB compression
        erb = stream_model.erb
        feat_erb = erb.bm(feat)  # (1, 3, 1, 129)

        # SFE
        sfe = stream_model.sfe
        feat_sfe = sfe(feat_erb)  # (1, 9, 1, 129)

        # Encoder conv 0
        en_out0 = stream_model.encoder.en_convs[0](feat_sfe)  # (1, 16, 1, 65)

        print("=== EnConv0 output (16, 65) ===")
        print(f"Shape: {en_out0.shape}")  # Should be (1, 16, 1, 65)
        en_out0_np = en_out0[0, :, 0, :].numpy()  # (16, 65)

        print(f"\nChannel 8 first 10 values: {en_out0_np[8, :10]}")
        print(f"Channel 9 first 10 values: {en_out0_np[9, :10]}")

        # Now trace EnConv1 manually
        en_conv1_block = stream_model.encoder.en_convs[1]
        conv_layer = en_conv1_block.conv

        print(f"\n=== EnConv1 Conv2d params ===")
        print(f"in_channels: {conv_layer.in_channels}")
        print(f"out_channels: {conv_layer.out_channels}")
        print(f"kernel_size: {conv_layer.kernel_size}")
        print(f"stride: {conv_layer.stride}")
        print(f"padding: {conv_layer.padding}")
        print(f"groups: {conv_layer.groups}")

        # Grouped conv: group 0 processes in_ch 0-7 -> out_ch 0-7
        #               group 1 processes in_ch 8-15 -> out_ch 8-15
        # For output channel 8 (first of group 1), we only use input channels 8-15!

        weight = conv_layer.weight.detach().numpy()  # (16, 8, 1, 5)
        bias = conv_layer.bias.detach().numpy()      # (16,)

        print(f"\nWeight shape: {weight.shape}")
        print(f"Bias shape: {bias.shape}")

        # Manual convolution for oc=8, of=0
        oc = 8  # Output channel (first of group 1)
        of = 0  # Output frequency
        stride = 2
        pad = 2  # (0, 2) padding in freq dimension

        print(f"\n=== Manual computation for oc={oc}, of={of} ===")
        print(f"Bias[{oc}] = {bias[oc]:.6f}")

        # Group 1 uses input channels 8-15
        # In PyTorch grouped conv, weight[oc, ic, ...] where ic is LOCAL (0-7)
        # but it accesses INPUT channel (group * in_ch_per_group + ic)

        sum_val = bias[oc]
        group = 1
        in_ch_per_group = 8

        for ic_local in range(in_ch_per_group):
            ic_abs = group * in_ch_per_group + ic_local  # 8 + ic_local
            contrib = 0

            print(f"\nic_local={ic_local} (ic_abs={ic_abs}):", end="")
            for kf in range(5):  # kernel_f = 5
                # Input frequency index: of * stride - pad + kf = 0 * 2 - 2 + kf = kf - 2
                if_idx = of * stride - pad + kf
                if 0 <= if_idx < 65:
                    inp_val = en_out0_np[ic_abs, if_idx]
                    wt_val = weight[oc, ic_local, 0, kf]  # Note: ic_local, not ic_abs!
                    contrib += inp_val * wt_val
                    print(f" kf={kf} if_idx={if_idx} w={wt_val:.4f} inp={inp_val:.4f};", end="")

            sum_val += contrib
            print(f"\n  contrib={contrib:.6f}, sum={sum_val:.6f}")

        print(f"\nFinal result for oc=8, of=0: {sum_val:.6f}")

        # Now run actual conv to verify
        conv_out = conv_layer(en_out0)
        print(f"\nActual conv output at [0, 8, 0, 0]: {conv_out[0, 8, 0, 0].item():.6f}")

if __name__ == "__main__":
    main()
