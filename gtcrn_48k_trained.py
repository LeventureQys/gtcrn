"""
GTCRN 48kHz Inference Script
用于语音增强的推理脚本

使用方法:
    # 处理单个文件
    python gtcrn_inference.py --model_path model.tar --input audio.wav --output enhanced.wav
    
    # 批量处理文件夹
    python gtcrn_inference.py --model_path model.tar --input_dir ./noisy_wavs --output_dir ./enhanced_wavs
"""
import os
import argparse
import torch
...
class GTCRN(nn.Module):
    def __init__(
        ...
        # 48kHz配置:
        # n_fft=1536 -> nfreqs=769
        # ERB: erb_subband_1=195, erb_subband_2=190
        # Total ERB bins: 195 + 190 = 385
        # After 2x freq downsampling in encoder: 385/4 = 96.25 -> 97
        self.erb = ERB(195, 190, nfft=n_fft, high_lim=24000, fs=48000)
        self.sfe = SFE(3, 1)

        self.encoder = Encoder()

        self.dpgrnn1 = DPGRNN(16, 97, 16, dropout=dropout)
        self.dpgrnn2 = DPGRNN(16, 97, 16, dropout=dropout)
    ...
    def forward(self, x):
        ...
        feat = self.dpgrnn1(feat)  # (B,16,T,97)
        feat = self.dpgrnn2(feat)  # (B,16,T,97)
...
class GTCRNInference:
    ...
    def load_audio(self, audio_path: str) -> tuple:
        ...
        if sr != self.sample_rate:
            print(f"  警告: 输入音频采样率为 {sr}Hz，正在重采样到 {self.sample_rate}Hz")
            try:
                import librosa
...
def main():
    parser = argparse.ArgumentParser(
        description='GTCRN 48kHz 语音增强推理',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 处理单个文件
  python gtcrn_inference.py -m model.tar -i noisy.wav -o enhanced.wav
  
  # 批量处理文件夹
  python gtcrn_inference.py -m model.tar --input_dir ./noisy --output_dir ./enhanced
  
  # 批量处理并添加后缀
  python gtcrn_inference.py -m model.tar --input_dir ./noisy --output_dir ./enhanced --suffix "_clean"


  python gtcrn_48k_trained.py --input_dir ./test_wavs/input --output_dir ./test_wavs/output 
        """
    )
    
    # 模型参数
    parser.add_argument('--model_path', '-m', type=str, 
                        default="./onnx_models/best_model_045-5db-20db.tar",
    ...
    # 创建推理器
    inferencer = GTCRNInference(args.model_path, args.device)
